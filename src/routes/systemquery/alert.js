...
let ds = ['ds1', 'ds2', 'ds3', 'ds4', 'ds5', 'ds6', 'ds7']
...
export default class Index extends React.Component {
  ...
  componentWillMount () {
    ...
    this.setState({
      chartHeight: (config.queryConfig.length + 2) * 30,
    })
  }
  ...
  initChart (el) {
    ...
    option.yAxis.data = config.queryConfig.map(q => q.name)

    if (el) {
      this.chart = echarts.init(el)
      this.chart.setOption(option)
      this.chart.on('click', e => this.query(e))
    } else {
