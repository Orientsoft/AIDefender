...
export default class Index extends React.Component {
  ...
  componentWillMount () {
    ...
    this.setState({
      chartHeight: (config.kpiConfig.length + 1) * 30,
    })
  }

  initChart (el) {
    const { app: { globalTimeRange }, config } = this.props
    const from = globalTimeRange[0]
    const to = globalTimeRange[1]
    const days = []

    for (let start = from.clone(); start.isBefore(to); start = start.add(1, 'days')) {
      days.push(start.clone())
    }
    option.xAxis.data = days.map(d => d.format('YYYY-MM-DD'))
    option.yAxis.data = config.kpiConfig.map(q => q.name)
    ...
    if (el) {
      ...
      this.chart.on('click', ({ data }) => {
        const x = data[0]
        const timeRange = [days[x], days[x + 1]]
        const kpi = config.kpiConfig[data[1]]
        this.query(kpi, timeRange)
      })
    } else {
